...
export default function FeaturedStays() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchFeaturedStays(9);
  ...
  return (
    <div>
      <Seo
        title="Featured Stays · CheapStays"
        description="Promoted and top-rated owner-direct stays across the Philippines — discounted and highest-rated places with standout value."
        path="/featured-stays"
      />
        ...
        <section className="container py-16 md:py-20">
          <div className="max-w-2xl">
            ...
            <h1 className="text-4xl md:text-5xl font-semibold tracking-tight leading-[1.05]">
              Promoted &amp; top-rated, <span className="text-accent">handpicked for value</span>.
            </h1>
            <p className="mt-4 text-muted-foreground">
              Discounted and highest-rated owner-direct places on CheapStays right now — the kind of stays travelers keep coming back to.
            </p>
          </div>

          {!loading && stays.length === 0 ? (
            <div className="mt-12 flex flex-col items-center justify-center text-center py-24">
              <div className="h-16 w-16 rounded-2xl bg-secondary/60 ring-1 ring-border/60 grid place-items-center mb-5">
                <Sparkles className="h-8 w-8 text-muted-foreground" />
              </div>
              <h2 className="text-xl font-medium tracking-tight">No featured stays yet</h2>
              <p className="mt-2 max-w-md text-sm text-muted-foreground">
                Once hosts publish and promote listings, the standout stays will appear here automatically.
              </p>
                        ...
                        <div className="p-5 space-y-2">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <h2 className="text-lg font-medium tracking-tight line-clamp-1">{s.title}</h2>
