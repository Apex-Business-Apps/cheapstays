...
export default function PopularCities() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchPopularCities(8);
  ...
  return (
    <div>
      <Seo
        title="Popular Cities · CheapStays"
        description="Explore the most-booked Philippine cities and towns on CheapStays — from Cebu to Coron."
        path="/popular-cities"
      />
        ...
        <section className="container py-16 md:py-20">
          <div className="max-w-2xl">
            ...
            <h1 className="text-4xl md:text-5xl font-semibold tracking-tight leading-[1.05]">
              Where travelers are <span className="text-accent">booking now</span>.
            </h1>
            <p className="mt-4 text-muted-foreground">
              Browse the destinations with the most owner-direct stays. Tap any city to see live listings.
            </p>
          </div>
          ...
          {!loading && cities.length === 0 ? (
            <div className="mt-12 flex flex-col items-center justify-center text-center py-24">
              ...
              <h2 className="text-xl font-medium tracking-tight">No cities to show yet</h2>
              <p className="mt-2 max-w-md text-sm text-muted-foreground">
                As hosts publish listings, the most popular destinations will appear here automatically.
              </p>
            ...
            <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
              {loading ? (
                [1, 2, 3, 4, 5, 6, 7, 8].map((n) => <CityCardSkeleton key={n} />)
              ) : (
                cities.map((c, idx) => {
                ...
                return (
                  ...
                  >
                    <Card className="group relative overflow-hidden border-border/60 bg-card/95 p-0 h-full transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_30px_80px_-40px_hsl(150_30%_10%/0.5)]">
                      ...
                      <div className="p-5">
                        <h2 className="text-lg font-medium tracking-tight">{c.city}</h2>
