...
import { fetchLatestListings, type DiscoveryListing } from "@/lib/discovery";
...
type Slide = { src: string; label: string; price: string; to?: string };

// Curated fallback — used while loading or when no live listings exist yet.
const FALLBACK_STAYS: Slide[] = [
...
export const HERO_STAYS_COUNT = FALLBACK_STAYS.length;
const INTERVAL = 30_000;

function toSlide(l: DiscoveryListing): Slide | null {
  const src = getListingPrimaryImage(l);
  if (!src) return null;
  const where = [l.city, l.province].filter(Boolean).join(", ");
  return {
    src,
    label: where ? `${l.title} — ${where}` : l.title,
    price: l.nightly_php > 0 ? `₱${l.nightly_php.toLocaleString()}/night` : "Owner-direct pricing",
    to: l.slug ? `/listing/slug/${l.slug}` : `/listing/${l.id}`,
  };
}

export function HeroCarousel() {
  const [slides, setSlides] = useState<Slide[]>(FALLBACK_STAYS);
  const [i, setI] = useState(0);

  // Pull the newest active listings; fall back to the curated set on empty/error.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const data = await fetchLatestListings(10);
        if (cancelled) return;
        const mapped = data.map(toSlide).filter((s): s is Slide => s !== null);
        if (mapped.length > 0) {
          setSlides(mapped);
          setI(0);
        }
    ...
    <>
      ...
      >
        <div>
          ...
          <p className="text-lg font-medium line-clamp-1">{active.label}</p>
        </div>
        <span className="shrink-0 rounded-full bg-background/90 px-3 py-1 text-xs font-medium text-foreground backdrop-blur">{active.price}</span>
      </motion.div>
    </>
  );

  return (
    <div className="relative aspect-[5/4] sm:aspect-[1/1] w-full overflow-hidden rounded-3xl shadow-[0_30px_80px_-30px_hsl(150_30%_10%/0.35)] ring-1 ring-border/60 bg-muted">
      ...
      {active.to ? (
        <Link to={active.to} className="absolute inset-0 z-10" aria-label={`View ${active.label}`}>{Meta}</Link>
      ) : (
        Meta
      )}
      ...
      <div className="absolute top-4 right-4 z-20 flex gap-1.5">
        {slides.map((_, idx) => (
          <button
            ...
            onClick={() => setI(idx)}
            className={`h-1.5 rounded-full transition-all duration-500 ${idx === i ? "w-6 bg-background" : "w-1.5 bg-background/50 hover:bg-background/80"}`}
          />
        ))}
      </div>
    </div>
