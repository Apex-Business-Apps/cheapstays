...
2. **P2 - Manual Cloudflare deploy required:** Cloudflare Pages does not auto-deploy on GitHub merge. Every production release requires manual execution of `npm run release:production`. Risk of stale frontend if this step is missed.
3. **P3 - Bundle size warning:** Main JS bundle exceeds the 500KB warning threshold in production build. Code splitting has not been applied.
4. **Policy regression risk:** No automated CI database-level RLS test harness yet.
5. **Operational drift risk:** Manual role and token lifecycle processes need periodic review.
